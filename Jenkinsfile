...
def deploy_service (cluster, zone, image_id, namespace) {
  ...
  docker.image('buildpack-deps:jessie-scm').inside {
    ...
    sh("kubectl --namespace=${namespace} rollingupdate gceme-frontend --image=${image_id}")
    sh("kubectl --namespace=${namespace}  rollingupdate gceme-backend --image=${image_id}")
    sh("echo http://`kubectl --namespace=${namespace}  get service/gceme-frontend --output=json | jq -r '.status.loadBalancer.ingress[0].ip'` > lb_url")
